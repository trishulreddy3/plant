...
const AddStaff = ({ embedded = false, onBack }: { embedded?: boolean; onBack?: () => void }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // normalize phone with +91 prefix (store with country code)
      const raw = String(formData.phoneNumber || '').trim();
      const digits = raw.replace(/\D/g, '');
      const normalized = digits.replace(/^(91)/, '');
      const phoneWithCode = `+91 ${normalized}`;
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br from-primary/5 via-background to-secondary/5 ${embedded ? 'p-0' : 'p-4'}`}>
      {!embedded && (
        <div className="absolute top-4 left-4 z-10">
          <BackButton />
        </div>
      )}
      <div className={`container max-w-2xl mx-auto ${embedded ? 'py-0' : 'py-8'}`}>
        {!embedded && (
          <Button
            variant="ghost"
            ...
            {!generatedPassword ? (
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="p-3 bg-muted rounded-lg">
                    <p className="font-mono">{`+91 ${String(formData.phoneNumber || '').replace(/\D/g, '')}`}</p>
                  </div>
                </div>
